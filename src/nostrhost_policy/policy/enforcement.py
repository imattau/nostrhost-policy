...
NOTE (library copy): the MCP-transport coupling of the reference's
enforcement.py has been removed. `translate_known_errors` (which mapped
these errors onto the MCP SDK's ToolError) and the yunohost-adapter
exception imports stay in the reference implementation; this library
provides only the framework-free `require_scope` / `require_confirmation`
enforcement and `ScopeError`. The reference keeps its own full copy.
"""
