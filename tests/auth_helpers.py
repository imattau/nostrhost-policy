...
sign_event() itself is imported from nostrhost_policy.auth.nostr - the real,
...
def make_delegation_event(
    ...
    *,
    ...
    from nostrhost_policy.auth.delegation import DELEGATION_KIND
...
def make_owner_approval_event(
    ...
    *,
    ...
    from nostrhost_policy.policy.confirmation import OWNER_APPROVAL_KIND

    created_at = int(time.time()) if created_at is None else created_at
    tags = [["confirmation_id", confirmation_id], ["operation_hash", operation_hash]]
    return sign_event(sk, pubkey=pubkey, created_at=created_at, kind=kind if kind is not None else OWNER_APPROVAL_KIND, tags=tags)
