...
from nostrhost_policy.auth.identity import LOCAL_STDIO_REQUEST, set_current_request
from nostrhost_policy.policy.enforcement import require_confirmation
from nostrhost_policy.policy.rules import PolicyRule
from nostrhost_policy.policy.confirmation import ConfirmationError, ConfirmationStore, SQLiteConfirmationStore
...
def test_approve_rejects_invalid_signature():
    store = ConfirmationStore()
    ticket = store.create(pubkey="agent", tool="system.upgrade", arguments={}, plan={})
    owner_sk, owner_pk = new_keypair()
    forged = make_owner_approval_event(
    ...
    bad = forged.model_dump()
    bad["sig"] = "0" * 128
    with pytest.raises(ConfirmationError, match="signature"):
        store.approve(ticket.confirmation_id, owner_pubkey=owner_pk, signed_approval=bad)
...
def test_approve_unknown_id_rejected():
    ...
    owner_sk, owner_pk = new_keypair()
    signed = make_owner_approval_event(
        owner_sk, owner_pk, confirmation_id="confirm-nope", operation_hash="0" * 64
    )
    with pytest.raises(ConfirmationError):
